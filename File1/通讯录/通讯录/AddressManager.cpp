...
void AddressManager::start()
{
	while(1)
	{
		...
		if(n==4)
		{
			...
			if(i<=m_Reclist.length())
			{
				cout<<"姓名"<<m_Reclist.at(i).getName()<<endl;
				cout<<"电话"<<m_Reclist.at(i).getPhone()<<endl;
				cout<<"地址"<<m_Reclist.at(i).getAddress()<<endl;
			
			}
			else
			{
				cout<<"[联系人"<<name2<<"不存在！]"<<endl;
			}
